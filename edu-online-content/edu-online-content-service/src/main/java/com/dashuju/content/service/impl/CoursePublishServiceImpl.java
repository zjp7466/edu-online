...
import com.dashuju.base.execption.CommonError;
import com.dashuju.base.execption.EduOnlineException;
import com.dashuju.content.config.MultipartSupportConfig;
import com.dashuju.content.feignclient.MediaServiceClient;
import com.dashuju.content.mapper.CourseBaseMapper;
import com.dashuju.content.mapper.CourseMarketMapper;
import com.dashuju.content.mapper.CoursePublishMapper;
import com.dashuju.content.mapper.CoursePublishPreMapper;
...
import com.dashuju.content.model.po.CoursePublishPre;
...
public class CoursePublishServiceImpl implements CoursePublishService {
    ...
    public void commitAudit(Long companyId, Long courseId) {
        ...
        if("202003".equals(auditStatus)){
            EduOnlineException.cast("当前为等待审核状态，审核完成可以再次提交。");
        }
        //本机构只允许提交本机构的课程
        if(!courseBase.getCompanyId().equals(companyId)){
            EduOnlineException.cast("不允许提交其它机构的课程。");
        }

        //课程图片是否填写
        if(StringUtils.isEmpty(courseBase.getPic())){
            EduOnlineException.cast("提交失败，请上传课程图片");
        }
        //添加课程预发布记录
        ...
        if(teachplanTree.size()<=0){
            EduOnlineException.cast("提交失败，还没有添加课程计划");
        }
        //转json
    ...
    public void publish(Long companyId, Long courseId) {
        ...
        if(coursePublishPre == null){
            EduOnlineException.cast("请先提交课程审核，审核通过才可以发布");
        }
        //本机构只允许提交本机构的课程
        ...
        if(!"202004".equals(auditStatus)){
            EduOnlineException.cast("操作失败，课程审核通过方可发布。");
        }
        //保存课程发布信息
    ...
    private void saveCoursePublish(Long courseId){
        ...
        CoursePublishPre coursePublishPre = coursePublishPreMapper.selectById(courseId);
        if(coursePublishPre == null){
            EduOnlineException.cast("课程预发布数据为空");
        }
    ...
    public File generateCourseHtml(Long courseId) {
        ...
        try {
            ...
            EduOnlineException.cast("课程静态化异常");
        }
        return htmlFile;
    }
    @Autowired
    ...
    public void uploadCourseHtml(Long courseId, File file) {
        ...
        if(course==null){
            EduOnlineException.cast("上传静态文件异常");
        }
    }
